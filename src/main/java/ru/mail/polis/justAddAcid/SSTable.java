...
public class SSTable {
    ...
    private Row clusterAt(final int i) {
        assert 0 <= i && i < rows;
        ...
        if (timeStamp < 0) {
            return new Row(key.slice(), new RowValue(null, -timeStamp, true));
        } else {
            ...
            return new Row(key.slice(), new RowValue(value.slice(), timeStamp, false));
        }
    }
}
